...
public class BitacoraRepository implements IRepository<BitacoraMantenimiento> {
	...
	public void create(BitacoraMantenimiento b) {
		String sql = "INSERT INTO bitacora_mantenimiento (id_equipo, id_personal, fecha, descripcion) VALUES (?, ?, ?, ?)";
		try {
			PreparedStatement ps = conn.prepareStatement(sql);
			ps.setInt(1, b.getIdEquipo());
			ps.setInt(2, b.getIdPersonal());
			ps.setDate(3, new Date(b.getFecha().getTime()));
	...
	public List<BitacoraMantenimiento> readAll() {
		...
		try {
			...
			while (rs.next()) {
				lista.add(new BitacoraMantenimiento(rs.getInt("id_bitacora"), rs.getInt("id_equipo"),
						rs.getInt("id_personal"), rs.getDate("fecha"), rs.getString("descripcion")));
			}
			ps.close();
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public Optional<BitacoraMantenimiento> findById(int id) {
		String sql = "SELECT * FROM bitacora_mantenimiento WHERE id_bitacora = ?";
		try {
			PreparedStatement ps = conn.prepareStatement(sql);
			ps.setInt(1, id);
			ResultSet rs = ps.executeQuery();
			if (rs.next()) {
				BitacoraMantenimiento b = new BitacoraMantenimiento(rs.getInt("id_bitacora"), rs.getInt("id_equipo"),
						rs.getInt("id_personal"), rs.getDate("fecha"), rs.getString("descripcion"));
				ps.close();
	...
	public void update(BitacoraMantenimiento b) {
		String sql = "UPDATE bitacora_mantenimiento SET id_equipo=?, id_personal=?, fecha=?, descripcion=? WHERE id_bitacora=?";
		try {
			PreparedStatement ps = conn.prepareStatement(sql);
			ps.setInt(1, b.getIdEquipo());
			ps.setInt(2, b.getIdPersonal());
	...
	public void delete(int id) {
		String sql = "DELETE FROM bitacora_mantenimiento WHERE id_bitacora = ?";
		try {
			PreparedStatement ps = conn.prepareStatement(sql);
			ps.setInt(1, id);
			ps.executeUpdate();
