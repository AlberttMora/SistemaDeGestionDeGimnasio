...
public class MembresiaRepository implements IRepository<Membresia> {
	...
	public List<Membresia> readAll() {
		...
		try {
			...
			while (rs.next()) {
				Membresia m = new Membresia(rs.getInt("id_membresia"), rs.getString("tipo"), rs.getDouble("precio"),
	...
	public void update(Membresia m) {
		String sql = "UPDATE membresia SET tipo=?, precio=?, duracion_dias=? WHERE id_membresia=?";
		try {
			PreparedStatement ps = conn.prepareStatement(sql);
			ps.setString(1, m.getTipo());
			ps.setDouble(2, m.getPrecio());
