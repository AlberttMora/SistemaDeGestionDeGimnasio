...
public class EquipoRepository implements IRepository<Equipo> {
    ...
    public void create(Equipo e) {
        String sql = "INSERT INTO equipo (idTipo, nombre, estado, fotoRuta, fechaAdquisicion) VALUES (?, ?, ?, ?, ?)";
        try {
    ...
    public List<Equipo> readAll() {
        ...
        try {
            ...
            while (rs.next()) {
                Equipo e = new Equipo(rs.getInt("idEquipo"), rs.getInt("idTipo"), rs.getString("nombre"), rs.getString("estado"), rs.getString("fotoRuta"), rs.getDate("fechaAdquisicion"));
                lista.add(e);
            }
            ps.close();
        } catch (SQLException e1) {
    ...
    public void update(Equipo e) {
        String sql = "UPDATE equipo SET idTipo=?, nombre=?, estado=?, fotoRuta=?, fechaAdquisicion=? WHERE idEquipo=?";
        try {
            PreparedStatement ps = conn.prepareStatement(sql);
            ps.setInt(1, e.getIdTipo());
            ps.setString(2, e.getNombre());
