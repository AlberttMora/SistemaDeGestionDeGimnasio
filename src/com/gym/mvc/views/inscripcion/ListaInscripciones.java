...
public class ListaInscripciones extends JFrame {
	...
	private JPanel crearContenido() {
		...
		panelBusq.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(SEPARADOR),
		...
		cmbEstado.setBackground(FONDO_CAMPO);
		...
		String[] columnas = { "ID Inscripción", "ID Cliente", "Cliente", "ID Membresía", "Tipo Membresía",
				"Fecha Inicio", "Estado" };
		...
		tabla = new JTable(modelo) {
			...
			public Component prepareRenderer(TableCellRenderer r, int row, int col) {
				...
				if (isRowSelected(row)) {
					...
					if (col == 6)
						c.setForeground(estadoColor((String) getValueAt(row, 6)));
					else
