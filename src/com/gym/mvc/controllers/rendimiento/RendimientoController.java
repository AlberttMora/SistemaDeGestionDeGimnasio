...
public class RendimientoController extends BaseController {
	...
	private void cargarComboEquipos() {
		...
		try {
			...
			for (Equipo eq : equipos) {
				combo.addItem(String.valueOf(eq.getIdEquipo()) + " - " + eq.getNombre());
			}
		} catch (Exception ex) {
	...
	private void cargarTablaAsignaciones() {
		...
		for (AsignacionEntrenador a : lista) {
			...
			modelo.addRow(new Object[] { a.getIdAsignacion(), a.getIdEntrenador(), nombreEntrenador, a.getIdCliente(),
					nombreCliente, formatoFecha.format(a.getFechaInicio()),
					a.getFechaFin() != null ? formatoFecha.format(a.getFechaFin()) : "", a.getObservaciones() });
		}
	...
	public void create() {
		try {
			...
			vistaAsignaciones.mostrarError("No se pudo guardar la asignacion");
		}
	}
	...
	private AsignacionEntrenador mapToEntity() throws ParseException {
		...
		a.setIdEntrenador(Integer.parseInt(idEntrenadorStr));
		a.setIdCliente(Integer.parseInt(idClienteStr));
		...
		if (!fechaFinStr.isEmpty()) {
			...
			a.setFechaFin(a.getFechaInicio());
		}
	...
	private void registrarPeso() {
		...
		try {
			...
			vistaAsignaciones.mostrarError("No se pudo registrar la metrica");
		}
	}
}