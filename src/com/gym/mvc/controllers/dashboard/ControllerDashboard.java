...
public class ControllerDashboard extends BaseController {
    ...
    private void aplicarPermisos() {
        ...
        if (session.isAdmin()) {
            configurarVisible("CLIENTES", "PAGOS", "RENDIMIENTO", "PERSONAL", "INVENTARIO", "OPERACIONES");
    ...
    private boolean tieneAcceso(UserSession.Rol rol, String modulo) {
        ...
        if (rol == UserSession.Rol.ADMIN)      return true;

        if (rol == UserSession.Rol.ENTRENADOR) {
            return !modulo.equals("PERSONAL") && !modulo.equals("OPERACIONES");
        }

        if (rol == UserSession.Rol.LIMPIEZA) {
            return modulo.equals("INVENTARIO") || modulo.equals("OPERACIONES");
        }

        if (rol == UserSession.Rol.CLIENTE) {
            return modulo.equals("PAGOS") || modulo.equals("INVENTARIO");
        }
